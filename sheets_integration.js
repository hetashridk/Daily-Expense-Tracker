// Daily Sales Telegram Bot - FIXED GOOGLE FORM SUBMISSION
// With proper axios headers for modern Google Forms
...
// FIXED: Submit single item to Google Form with proper headers
async function submitToGoogleForm(date, itemName, amount, time) {
  try {
    ...
    // Create URLSearchParams (not URLSearchParams)
    const data = new URLSearchParams();
    data.append('entry.1192946452', date);           // DATE
    data.append('entry.682169264', itemName);        // ITEM NAME
    data.append('entry.1195426964', String(amount)); // AMOUNT (convert to string)
    data.append('entry.94204135', time);             // TIME

    console.log(`📤 Submitting to Google Form...`);
    console.log(`   Item: ${itemName} | Amount: ₹${amount} | Date: ${date}`);

    const response = await axios.post(formUrl, data.toString(), {
      headers: { 
      ...
      validateStatus: function (status) {
        // Accept 200-299 or 303 (Google Forms redirect)
    ...
    // Google Forms might throw error but still save the data
    // This is expected behavior with no-CORS submissions
    console.log(`📨 Form submission sent (${error.message})`);
    console.log(`   Note: Google Forms may still receive the data`);
...
async function addSalesItem(userId, itemName, amount) {
  ...
  console.log(`💾 Saved locally: ${itemName} (₹${amount})`);
  
  // Then sync to Google Form (fire and forget)
